...
$texto = " FORMAT(SUM(m.valor), 2, 'de_DE') AS total";
...
$mensal = Movimentacao::getList('sum(m.valor) as total', 'movimentacoes as m ', 
'month(m.data) = MONTH(CURRENT_DATE()) AND m.tipo=1 AND m.status=1', null, null);
...
$total_mes = $geral_mes - $despesadia;
...
$invent = Movimentacao::getList('p.valor_venda as venda,p.estoque as estoque', 'produtos as p ','p.estoque > 0', null, null);
...
foreach ($invent as $value) {
    ...
    $valor_total_compra += $value->venda;
    $valor_total_quantidade += $value->estoque;
    
    $total_intario += $value->venda * $value->estoque;
}
...
END) AS mes, SUM(m.valor) as total', 'movimentacoes as m group by month(m.data)', null, null, null);
